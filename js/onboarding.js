...
export function _setupSave() {
  ...
  // Hide setup and enter directly. The short intro now belongs to first open.
  ...
  if (typeof window.render === 'function') setTimeout(() => { window.render(); }, 120);
}
...
export function _skipSetup() {
  ...
  if (typeof window.render === 'function') setTimeout(() => { window.render(); }, 100);
}
...
export function skipCinematic() { _finishCinematic(true); }
